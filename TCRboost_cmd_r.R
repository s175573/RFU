...
PrepareAdaptiveFile <- function(indir,outdir,AlternativeFormat=FALSE,thr=10000, naive=FALSE, pattern='.tsv', randomize=TRUE){
  ...
  for(ff in ffs){
    ...
    if(AlternativeFormat){
      ...
      ii.ff=grep('[max_]*productive_frequency',colnames(ddnew))
    }else{
      ii.aa=2
      ii.vv=6
      ii.ff=4
    ...
    #ss=sum(ddnew[,3])
    #ddnew[,4]=ddnew[,3]/ss
    if(naive)ddnew=ddnew[order(as.numeric(ddnew[,ii.ff])),] else ddnew=ddnew[order(as.numeric(ddnew[,ii.ff]),decreasing=T),]
    ...
    if(nrow(ddnew)>thr){
      if(randomize){
        freq.t = ddnew[thr,ii.ff]
        vv.t = which(ddnew[,ii.ff]==freq.t)
        vv.tr = sample(vv.t, length(vv.t))  ## permutate TCRs with at-threshold frequency
        ddnew[vv.t, ] = ddnew[vv.tr, ]
      }
      ddnew=ddnew[1:thr,c(ii.aa,ii.vv,ii.ff)] 
    }else ddnew=ddnew[,c(ii.aa,ii.vv,ii.ff)]
    #ddnew=ddnew[1:5000,]
  ...
  for(ff in ffs){
    ...
    if(nrow(ddnew)>thr){
      if(randomize){
        ...
        vv.t = which(ddnew[,ii.ff]==freq.t)
        if(clip){
          ddnew = ddnew[1:max(vv.t),]
        }else{
          #vv.tr = sample(vv.t, length(vv.t))  ## permutate TCRs with at-threshold frequency
          vv.tr= vv.t[order(ddnew[vv.t, ii.aa], decreasing=T)]
          ddnew[vv.t, ] = ddnew[vv.tr, ]
        }
      }
    }
    if(!clip)ddnew=ddnew[1:min(thr, nrow(ddnew)),c(ii.aa,ii.vv,ii.ff)] else ddnew=ddnew[,c(ii.aa, ii.vv, ii.ff)]
    #ddnew=ddnew[1:min(thr,nrow(ddnew)),]
    tmp.nn=nchar(ddnew[,1])
    tmp.vv=which(tmp.nn>=10 & tmp.nn<=24)
    #tmp.vv=which(tmp.nn>=1)
    ddnew=ddnew[tmp.vv,]
    #ddnew=ddnew[which(ddnew[,ii.vv]!='unresolved'),]
